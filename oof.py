...
MAX_REPEAT_TOKEN_LENGTH = 10 # Maximum length of the string which to repeat.
...
def mutate_existing(data: bytes) -> bytes:
	...
	if c(REPLACE_CHANCE) and substr:
		...
		data = data[:place_index] + multiplication + data[place_index+len(substr):]
		#print("data == "+str(data))
